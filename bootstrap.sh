...
# Sleep a bit to allow the CRD to be fully registered before we query it
sleep 10
kubectl wait --for=condition=Ready kafkaconnect/cdc-connect -n kafka --timeout=300s || echo "Kafka Connect might still be starting, moving on..."
